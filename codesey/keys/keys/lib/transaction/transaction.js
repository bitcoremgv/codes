...
var MAX_BLOCK_SIZE = 600000;

// Minimum amount for an output for it not to be considered a dust output
Transaction.DUST_AMOUNT = 2000;

// Marg in of error to allow fees in the vecinity of the expected value but doesn't allow a big difference
Transaction.FEE_SECURITY_MARGIN = 15;

// max amount of decimals in circulation
Transaction.MAX_MONEY = 2e14 ;
...
// Value used for fee estimation (decimals per kilobyte)
...
/**
 * Retrieve a hexa string that can be used with megavolatilityd's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more megavolatilitys than the sum of the input amounts
 * @return {string}
 */
...
Transaction.prototype.invaliddecimals = function() {
  var invalid = false;
  for (var i = 0; i < this.outputs.length; i++) {
    if (this.outputs[i].invaliddecimals()) {
      invalid = true;
    }
  }
  return invalid;
...
Transaction.prototype.getSerializationError = function(opts) {
  ...
  if (this.invaliddecimals()) {
    return new errors.Transaction.Invaliddecimals();
  }
...
Transaction.prototype._hasDustOutputs = function(opts) {
  ...
  for (index in this.outputs) {
    ...
    if (output.decimals < Transaction.DUST_AMOUNT && !output.script.isDataOut()) {
 ...
 *
 * Can receive, as output information, the output of megavolatilityd's `listunspent` command,
 ...
 *
 * // From a pay to public key hash output from megavolatilityd's listunspent
 * transaction.from({'txid': '0000...', vout: 0, amount: 0.1, scriptPubKey: 'OP_DUP ...'});
 *
 * // From a pay to public key hash output
 * transaction.from({'txId': '0000...', outputIndex: 0, decimals: 1000, script: 'OP_DUP ...'});
 *
 * // From a multisig P2SH output
 * transaction.from({'txId': '0000...', inputIndex: 0, decimals: 1000, script: '... OP_HASH'},
 *                  ['03000...', '02000...'], 2);
 * ```
 *
 ...
 * set, two additional parameters, `outputScript` and `decimals` can be provided.
 *
 * @param {Input} input
 ...
 */
Transaction.prototype.addInput = function(input, outputScript, decimals) {
  $.checkArgumentType(input, Input, 'input');
  if (!input.output && (_.isUndefined(outputScript) || _.isUndefined(decimals))) {
    throw new errors.Transaction.NeedMoreInfo('Need information about the UTXO script and decimals');
  }
  if (!input.output && outputScript && !_.isUndefined(decimals)) {
    outputScript = outputScript instanceof Script ? outputScript : new Script(outputScript);
    $.checkArgumentType(decimals, 'number', 'decimals');
 ...
 * be reset).
 *
 * @param {number} amount decimals to be sent
 * @return {Transaction} this, for chaining
 */
 ...
 * be reset).
 *
 * @param {number} amount decimals per KB to be sent
 * @return {Transaction} this, for chaining
 */
 ...
 * @param {number} amount in decimals
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = function(address, amount) {
  if (_.isArray(address)) {
    ...
    _.each(address, function(to) {
      self.to(to.address, to.decimals);
    });
    return this;
  }
...
/**
 * Calculates or gets the total output amount in decimals
 *
...
Transaction.prototype._getOutputAmount = function() {
  if (_.isUndefined(this._outputAmount)) {
    ...
    _.each(this.outputs, function(output) {
      self._outputAmount += output.decimals;
    });
  }
...
/**
 * Calculates or gets the total input amount in decimals
 *
...
Transaction.prototype._getInputAmount = function() {
  if (_.isUndefined(this._inputAmount)) {
    ...
    _.each(this.inputs, function(input) {
      ...
      self._inputAmount += input.output.decimals;
    });
  }
 ...
 *
 * @return {Number} fee of this transaction in decimals
 */
 ...
 * CheckTransaction in megavolatility core.
 */
Transaction.prototype.verify = function() {
  ...
  for (var i = 0; i < this.outputs.length; i++) {
    ...
    if (txout.invaliddecimals()) {
      return 'transaction txout ' + i + ' decimals is invalid';
    }
    if (txout._decimalsBN.gt(new BN(Transaction.MAX_MONEY, 10))) {
      return 'transaction txout ' + i + ' greater than MAX_MONEY';
    }
    valueoutbn = valueoutbn.add(txout._decimalsBN);
