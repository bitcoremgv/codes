# Public Key
Represents a megavolatility public key and is needed to be able to receive megavolatility, as is usually represented as a megavolatility [Address](address.md). See the official [MegaVolatility Wiki](https://en.megavolatility.it/wiki/Technical_background_of_version_1_MegaVolatility_addresses).
...
A public key point should be on the [secp256k1](https://en.megavolatility.it/wiki/Secp256k1) curve, instantiating a new PublicKey will validate this and will throw an error if it's invalid. To check that a public key is valid:
