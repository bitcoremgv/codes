...
describe('Transaction', function() {
  ...
  it('can serialize to a plain javascript object', function() {
    ...
    object.inputs[0].output.decimals.should.equal(testAmount);
    ...
    object.outputs[0].decimals.should.equal(testAmount - 10000);
  });
  ...
  it('toObject/fromObject with signatures and custom fee', function() {
    var tx = new Transaction()
      .from(simpleUtxoWith100000decimals)
      .to([{address: toAddress, decimals: 50000}])
      .fee(15000)
  ...
  it('toObject/fromObject with p2sh signatures and custom fee', function() {
    var tx = new Transaction()
      .from(p2shUtxoWith1MMGV, [p2shPublicKey1, p2shPublicKey2, p2shPublicKey3], 2)
      .to([{address: toAddress, decimals: 50000}])
      .fee(15000)
      .change(changeAddress)
  ...
  it('fromObject with pay-to-public-key previous outputs', function() {
    ...
    tx.inputs[0].output.decimals.should.equal(5000000000);
  ...
  var simpleUtxoWith100000decimals = {
    address: fromAddress,
  ...
  var anyoneCanSpendUTXO = JSON.parse(JSON.stringify(simpleUtxoWith100000decimals));
  ...
  describe('change address', function() {
    it('can calculate simply the output amount', function() {
      ...
      transaction.outputs[1].decimals.should.equal(49000);
    ...
    it('can recalculate the change amount', function() {
      ...
      transaction.getChangeOutput().decimals.should.equal(50000);
      ...
      transaction.outputs[2].decimals.should.equal(30000);
    ...
    it('fee can be set up manually', function() {
      var transaction = new Transaction()
        ...
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].decimals.should.equal(10000);
    });
    it('fee per kb can be set up manually', function() {
      var inputs = _.map(_.range(10), function(i) {
        var utxo = _.clone(simpleUtxoWith100000decimals);
        utxo.outputIndex = i;
        return utxo;
      });
      var transaction = new Transaction()
      ...
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].decimals.should.equal(34000);
    });
    it('if decimals are invalid', function() {
      var transaction = new Transaction()
        .from(simpleUtxoWith100000decimals)
        .to(toAddress, 99999)
        .change(changeAddress)
        .sign(privateKey);
      transaction.outputs[0]._decimals = 100;
      transaction.outputs[0]._decimalsBN = new BN(101, 10);
      expect(function() {
        return transaction.serialize();
      }).to.throw(errors.Transaction.Invaliddecimals);
    });
  ...
  describe('checked serialize', function() {
    ...
    it('checks output amount before fee errors', function() {
      var transaction = new Transaction();
      transaction.from(simpleUtxoWith1MMGV);
      transaction
    ...
    describe('skipping checks', function() {
      ...
      it('can skip the check that avoids spending more megavolatilitys than the inputs for a transaction', buildSkipTest(
        function(transaction) {
          return transaction
            .to(toAddress, 10000000000000)
            .change(changeAddress)
  ...
  describe('#verify', function() {
    ...
    it('not if _decimals and _decimalsBN have different values', function() {
      var tx = new Transaction()
        .from({
          'txId': testPrevTx,
          'outputIndex': 0,
      ...
      tx.outputs[0]._decimals = 100;
      tx.outputs[0]._decimalsBN = new BN('fffffffffffffff', 16);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 decimals is invalid');
    });

    it('not if _decimals is negative', function() {
      var tx = new Transaction()
        .from({
          'txId': testPrevTx,
          'outputIndex': 0,
      ...
      tx.outputs[0]._decimals = -100;
      tx.outputs[0]._decimalsBN = new BN(-100, 10);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 decimals is invalid');
    });
  ...
  describe('removeInput and removeOutput', function() {
    it('can remove an input by index', function() {
      ...
      transaction.inputAmount.should.equal(simpleUtxoWith1MMGV.decimals);
    ...
    it('can remove an input by transaction id', function() {
      ...
      transaction.inputs.length.should.equal(1);
      transaction.inputAmount.should.equal(simpleUtxoWith1MMGV.decimals);
      transaction.removeInput(simpleUtxoWith1MMGV.txId, simpleUtxoWith1MMGV.outputIndex);
      transaction.inputs.length.should.equal(0);
      transaction.inputAmount.should.equal(0);
    });
    ...
    it('an output can be removed by index', function() {
      var transaction = new Transaction()
        .to([
          {address: toAddress, decimals: 40000000},
          {address: toAddress, decimals: 40000000}
        ])
      transaction.outputs.length.should.equal(2);
  ...
  describe('output ordering', function() {
    ...
    beforeEach(function() {
      transaction = new Transaction()
        .from(simpleUtxoWith1MMGV)
        .to([
          {address: toAddress, decimals: tenth},
          {address: toAddress, decimals: fourth}
        ])
        .to(toAddress, half)
        .change(changeAddress);
  ...
  describe('clearOutputs', function() {
    ...
    it('removes all outputs and maintains the transaction in order', function() {
      var tx = new Transaction()
        ...
        .to([
          {address: toAddress, decimals: fourth},
          {address: toAddress, decimals: half}
        ])
        .change(changeAddress);
      tx.clearOutputs();
      ...
      tx.outputs[0].decimals.should.equal(10000000);
      tx.outputs[0].script.toAddress().toString().should.equal(toAddress);
      tx.outputs[1].decimals.should.equal(89999000  );
  ...
  describe('BIP69 Sorting', function() {
    ...
    it('sorts outputs correctly', function() {
      ...
      tx.sort();
      tx.outputs[0].decimals.should.equal(1);
      tx.outputs[1].decimals.should.equal(2);
      tx.outputs[2].decimals.should.equal(2);
    ...
    describe('megavolatilityjs fixtures', function() {
        ...
        it(inputSet.description, function() {
          ...
          inputSet.inputs = inputSet.inputs.map(function(input) {
            var input = new Input({
              ...
              script: new Script(),
              output: new Output({ script: new Script(), decimals: 0 })
            });
    ...
    describe('#isRBF', function() {
      ...
      it('determine opt-out with 0xfffffffe', function() {
        var tx = new Transaction()
          .from(simpleUtxoWith1MMGV)
          .from(simpleUtxoWith100000decimals)
          .to([{address: toAddress, decimals: 50000 + 1e11}])
          .fee(15000)
          .change(changeAddress)
          .sign(privateKey);
...
var unsupportedTxObj = '{"version":1,"inputs":[{"prevTxId":"a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458","outputIndex":0,"sequenceNumber":4294967295,"script":"OP_1","output":{"decimals":1020000,"script":"OP_1 OP_ADD OP_2 OP_EQUAL"}}],"outputs":[{"decimals":1010000,"script":"OP_DUP OP_HASH160 20 0x7821c0a3768aa9d1a37e16cf76002aef5373f1a8 OP_EQUALVERIFY OP_CHECKSIG"}],"nLockTime":0}';
