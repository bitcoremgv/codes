...
describe('HDPrivate key interface', function() {
  ...
  var expectDerivationFail = function(argument, error) {
    return expectFail(function() {
      var privateKey = new HDPrivateKey(MGVL1key);
  ...
  describe('instantiation', function() {
    ...
    it('allows no-new calling', function() {
      HDPrivateKey(MGVL1key).toString().should.equal(MGVL1key);
    });
    it('allows the use of a copy constructor', function() {
      HDPrivateKey(HDPrivateKey(MGVL1key))
        .MGVL1key.should.equal(MGVL1key);
    });
  });
  ...
  describe('public key', function() {
    ...
    it('cache for MGVU1key works', function() {
      ...
      privateKey.MGVU1key.should.equal(privateKey.MGVU1key);
  ...
  it('inspect() displays correctly', function() {
    HDPrivateKey(MGVL1key).inspect().should.equal('<HDPrivateKey: ' + MGVL1key + '>');
  });
  ...
  it('allows derivation of hardened keys by passing a very big number', function() {
    ...
    derivedByNumber.MGVL1key.should.equal(derivedByArgument.MGVL1key);
  });
  ...
  it('returns InvalidNetworkArgument if an invalid network is provided', function() {
    expect(
      HDPrivateKey.getSerializedError(MGVL1key, 'invalidNetwork') instanceof errors.InvalidNetworkArgument
    ).to.equal(true);
  });

  it('recognizes that the wrong network was asked for', function() {
    expect(
      HDPrivateKey.getSerializedError(MGVL1key, 'testnetmgv') instanceof errors.InvalidNetwork
    ).to.equal(true);
  });

  it('recognizes the correct network', function() {
    expect(HDPrivateKey.getSerializedError(MGVL1key, 'livenetmgv')).to.equal(null);
  });

  describe('on creation from seed', function() {
    it('converts correctly from an hexa string', function() {
      should.exist(HDPrivateKey.fromSeed('01234567890abcdef01234567890abcdef').MGVL1key);
    });
  ...
  it('correctly errors if an invalid checksum is provided', function() {
    var privKey = new HDPrivateKey(MGVL1key);
    var error = null;
    try {
  ...
  it('shouldn\'t matter if derivations are made with strings or numbers', function() {
    ...
    derivedByNumber.MGVL1key.should.equal(derivedByString.MGVL1key);
  });
  ...
  describe('conversion to/from buffer', function() {
    ...
    it('should roundtrip to/from a buffer', function() {
      ...
      roundTrip.MGVL1key.should.equal(str);
    });
  });
  ...
  describe('conversion to plain object/json', function() {
    var plainObject = {
      ...
      'checksum': -411132559,
      'MGVL1key': 'xprv9s21ZrQH143K3QTDL4LXw2F7HEK3wJUD2nW2nRk4stbPy6cq3jPPqjiChkVvvN' +
    ...
    it('roundtrips toObject', function() {
      expect(HDPrivateKey.fromObject(new HDPrivateKey(MGVL1key).toObject()).MGVL1key).to.equal(MGVL1key);
    });
    it('roundtrips to JSON and to Object', function() {
      var privkey = new HDPrivateKey(MGVL1key);
      expect(HDPrivateKey.fromObject(privkey.toJSON()).MGVL1key).to.equal(MGVL1key);
    });
    it('recovers state from JSON', function() {
      new HDPrivateKey(JSON.stringify(plainObject)).MGVL1key.should.equal(MGVL1key);
    });
    it('recovers state from Object', function() {
      new HDPrivateKey(plainObject).MGVL1key.should.equal(MGVL1key);
    });
  });
});
